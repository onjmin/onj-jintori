...
| ファイル | 役割 |
|:---------|:-----|
| `jintori.js` | メインエントリ、ゲームループ、ラウンド管理 |
...
- 表示形式: `[チーム名] プレイヤー名`
- 制御文字 (0x00-0x1F, 0x7F) 禁止（違反時はリクエスト無視、BANしない）
- 未入力時: 「名無し」+ ランダム英数字2文字
...
- **ライン切断奪取統合**: jintori.jsの軌跡カットkill後の陣地奪取もループ外で一括処理 + rebuild1回
...
- プレイヤー名入力（8文字制限）
- 絵文字ピッカー（9カテゴリ: 顔/動物/食べ物/スポーツ/乗り物/モノ/記号/自然/人）
- 国旗絵文字セレクタ（30ヶ国）
...
- チャット入力（15文字制限、1ラウンド1回）+ コメント履歴ドロップダウン（最大5件、localStorage保存）
- 次ラウンドモード表示 + チーム選択UI
...
| # | 最適化 | 対象ファイル | 効果 |
|:--|:-------|:-------------|:-----|
| 22 | rebuildTerritoryRects呼び出し統合: 最大4回→1回 | game.js, jintori.js | ラグスパイク50-70%減 |
...
| 27 | ライン切断陣地奪取統合: ループ内個別rebuild→ループ外一括 | jintori.js | 複数kill時のrebuild回数削減 |
...
加えて `jintori.js` の軌跡カットkill後にもセグメントループ内で個別に呼ばれていた。
...
- **絵文字ピッカー**: ログイン画面にカテゴリ別絵文字ピッカーを追加。9カテゴリ（顔/動物/食べ物/スポーツ/乗り物/モノ/記号/自然/人）からプレイヤー絵文字を選択可能。選択はlocalStorageに保存され次回ログイン時に復元。「🎲 ランダムに戻す」ボタンでデフォルトに戻せる
- **コメント履歴**: チャット入力欄にコメント履歴ドロップダウンを追加。送信済みコメントを最大5件localStorageに保存し、入力欄フォーカス時に表示。タップで即送信、×ボタンで個別削除可能
- **エントリーポイントリネーム**: `server.v5.js` → `jintori.js` にリネーム

### 仕様変更

- **バリデーション緩和**: 名前の長さ超過・制御文字検出時の処理をBAN（IP禁止+切断）からリクエスト無視（ログ出力のみ）に変更。悪意のないミスでBANされる問題を解消
- **localStorage管理改善**: 名前・チーム・国旗が空の場合はlocalStorageから削除するよう変更（以前は空文字で保存されていた）

---

**End of Document v13.0.0**
